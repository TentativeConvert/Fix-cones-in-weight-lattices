...
newPackage(
	"WeylGroupsFixed",
	Version => "0.5.1-fixed",
	Date => "March 20, 2012 (fix: August 6, 2020)",
	Authors => {
...
--	AuxiliaryFiles => true,                       <------ REMOVE THIS COMMENT BEFORE INSTALLATION; THIS IS A HOTFIX!
...
rootSystem(RootSystem,Parabolic) := (R,P) ->
	...
	-- BUG: M:= if #P>0 then matrix(subtable(L,L,R.CartanMatrixTr)) else (matrix{{1}})^{}_{};
